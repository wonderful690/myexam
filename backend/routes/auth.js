...
const { generateTokenAndSetCookie, clearCookie, protect } = require('../middleware/authMiddleware');

const router = express.Router();

router.post('/register', [
  body('username').matches(/^[a-zA-Z0-9]{6,}$/).withMessage('Неверный формат логина'),
  ...
  try {
    ...
    const user = await User.create({ username, password, fullName, phone, email });
